  ...
  return (
    <div className="relative">
      <Button className={showDescription ? "h-auto min-h-20 w-full min-w-0 justify-start whitespace-normal px-4 py-3 text-left" : "h-11 min-w-36"} disabled={Boolean(savedItemId) || saving} onClick={handleSave} variant="outline" type="button">
        ...
        {showDescription ? (
          <span><span className="block">{saving ? "Saving..." : savedItemId ? "Saved" : label}</span><span className="mt-1 block text-xs font-normal text-muted-foreground">{label === "Save Place" ? "Keep this place for later" : "Keep this information for later"}</span></span>
        ) : saving ? "Saving..." : savedItemId ? "Saved" : label}
      </Button>
